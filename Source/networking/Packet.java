...
public class Packet 
{
	...
	/**
	 * Creates a new encrypted Packet from a String by encrypting the String using a secret key
	 */
	public Packet(String message, String secret)
	{
		packetBytes = new ArrayList<Byte>();
		...
		appendArrayToList(getPacketTypeInBytes(Type.ENCRYPTED_MESSAGE)); //Packet type
		
		appendArrayToList(SPACER);
		
		message = Encryption.encrypt(message, secret);
	...
	/**
	 * Sets a secret that can be used by the toString() method to decrypt the message
	 */
	...
	public String toString()
	{
		...
		else if (type == Type.ENCRYPTED_MESSAGE)
		{
			try
			{
				return "[Secure] " + Encryption.decrypt(new String(Utils.arrayToByteArray(Arrays.copyOfRange(packetBytes.toArray(), 17, packetBytes.size() - 8))), encryptionSecret);
			}
			catch (Exception e)
			{
